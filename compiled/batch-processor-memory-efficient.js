...
class MemoryEfficientBatchProcessor {
    ...
    async startBatchSession() {
        ...
        const url = `${this.config.apiBaseUrl}/api/${this.config.alias}/batch/start?strategy=${this.config.strategy}${this.config.estimatedTotal ? `&estimatedTotal=${this.config.estimatedTotal}` : ''}`;
     ...
     */
    async processProductsInBatchesStreaming() {
        ...
        return new Promise((resolve, reject) => {
            ...
            const processBatchWithConcurrencyControl = async (batch, batchNum) => {
    ...
    async processBatch(products, batchNumber) {
        ...
        const url = `${this.config.apiBaseUrl}/api/batch/${this.session.sessionId}/process`;
     ...
     */
    async completeBatchSession() {
        console.log('\n🏁 Completing batch session...');
        if (!this.session) {
            throw new Error('No active session');
        }
        const url = `${this.config.apiBaseUrl}/api/batch/${this.session.sessionId}/complete`;
        const response = await this.makeRequest('POST', url);
        if (!response.ok) {
            const error = await response.text();
            throw new Error(`Failed to complete batch session: ${response.status} - ${error}`);
        }
        const deploymentState = await response.json();
     ...
     */
    async promoteIndex(deploymentState) {
        console.log('\n🔄 Promoting index to active...');
        if (!deploymentState.stagingIndex) {
            throw new Error('No staging index to promote');
        }
        const url = `${this.config.apiBaseUrl}/api/${this.config.alias}/promote?targetIndex=${deploymentState.stagingIndex}`;
        const response = await this.makeRequest('POST', url);
        if (!response.ok) {
            const error = await response.text();
     ...
     */
    async cancelBatchSession() {
        if (!this.session) {
            return;
        }
        console.log('\n❌ Cancelling batch session...');
        const url = `${this.config.apiBaseUrl}/api/batch/${this.session.sessionId}/cancel`;
        try {
            const response = await this.makeRequest('POST', url);
            if (response.ok) {
